...
<body>
    <form action="{{ url('register-user') }}" method="post" style="border:1px solid #ccc; align-content:center">
      ...
      <div class="alert alert-success" style="color: green; align-content: center;">{{Session::get('success')}}</div>
      @endif
      @if(Session::has('fail'))
      <div class="alert alert-danger" style="color: red; align-content: center;">{{Session::get('fail')}}</div>
      @endif
      @csrf
        <div class="container">
          <h1>Sign Up</h1>
          ...
          <input type="text" placeholder="Enter Schoolname" name="school_name" required>
          <span class="text-danger">@error('school_name') {{$message}} @enderror</span>

          <label for="phone"><b>School Contact</b></label>
          <br>
          <input type="tel" placeholder="Enter Schoolcontact" name="contact" pattern="[0-9]{4}[0-9]{3}[0-9]{3}" required>
